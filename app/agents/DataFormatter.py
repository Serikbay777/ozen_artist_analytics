...
from app.agents.graph_instructions import graph_instructions
import logging
...
class DataFormatter:
    ...
    def format_data_for_visualization(self, state: dict) -> dict:
        ...
        logger.info("→ Шаг 8: Форматирование данных для визуализации")
        visualization = state['visualization'].strip().lower()  # Убираем пробелы и приводим к нижнему регистру
        results = state['results']
        question = state['question']
        sql_query = state['sql_query']

        if visualization == "none":
            logger.info("  Визуализация не требуется")
            return {"formatted_data_for_visualization": None}
        
        if visualization == "scatter":
            try:
                logger.info(f"  Форматирование scatter графика")
                result = self._format_scatter_data(results)
                logger.info(f"  ✓ Scatter график отформатирован, серий: {len(result['formatted_data_for_visualization']['series'])}")
                return result
            except Exception as e:
                logger.warning(f"  ⚠ Ошибка форматирования scatter: {str(e)}, используем общий метод")
        ...
        if visualization == "bar" or visualization == "horizontal_bar":
            try:
                logger.info(f"  Форматирование {visualization} графика")
                result = self._format_bar_data(results, question)
                data = result['formatted_data_for_visualization']
                logger.info(f"  ✓ Bar график отформатирован, категорий: {len(data['labels'])}, серий: {len(data['values'])}")
                return result
            except Exception as e:
                logger.warning(f"  ⚠ Ошибка форматирования bar: {str(e)}, используем общий метод")
                return self._format_other_visualizations(visualization, question, sql_query, results)
        
        if visualization == "line":
            try:
                logger.info(f"  Форматирование line графика")
                result = self._format_line_data(results, question)
                data = result['formatted_data_for_visualization']
                logger.info(f"  ✓ Line график отформатирован, точек: {len(data['xValues'])}, серий: {len(data['yValues'])}")
                return result
            except Exception as e:
                logger.warning(f"  ⚠ Ошибка форматирования line: {str(e)}, используем общий метод")
                return self._format_other_visualizations(visualization, question, sql_query, results)
        
        logger.info(f"  Использование общего метода форматирования для {visualization}")
        return self._format_other_visualizations(visualization, question, sql_query, results)
    ...
    def _format_other_visualizations(self, visualization, question, sql_query, results):
        ...
        try:
            ...
            logger.info(f"  ✓ Данные успешно отформатированы через общий метод")
            return {"formatted_data_for_visualization": formatted_data_for_visualization}
        except json.JSONDecodeError:
            logger.error(f"  ✗ Ошибка парсинга JSON ответа: {response[:200]}")
